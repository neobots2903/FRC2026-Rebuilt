...
/**
 * ============================================================================
 * STATIONARY SHOOTER KINEMATICS (COMPLETED)
 * ============================================================================
 * Calculates the exact turret angle and hood angle needed to hit the Hub
 * based purely on our robot's odometry (Pose2d) on the field.
 */
public class ShooterKinematics {
    ...
    private static final double TARGET_Z_METERS = 3.0; // Changed from 10 to 3 meters (~10 ft) for a realistic FRC target
    ...
    private static final double MIN_DESCENT_ANGLE_DEG = 20.0; // Minimum downward angle to fit through the rim
    ...
    private static final Translation2d TURRET_OFFSET_METERS = new Translation2d(
        Units.inchesToMeters(TURRET_OFFSET_X_IN), 
        Units.inchesToMeters(TURRET_OFFSET_Y_IN)
    );
    ...
    public static double getDistanceToTargetMeters(Pose2d robotPose) {
        ...
        return turretPos.getDistance(new Translation2d(TARGET_POSE_METERS.getX(), TARGET_POSE_METERS.getY()));
    }
    ...
    public static double getPhysicsHoodAngleDeg(Pose2d robotPose) {
        ...
        boolean angle1Valid = angle1 >= MIN_HOOD_ANGLE_DEG && angle1 <= MAX_HOOD_ANGLE_DEG 
                              && isTrajectoryValidForTopOpening(angle1, distance, exitVelocity);
                              
        boolean angle2Valid = angle2 >= MIN_HOOD_ANGLE_DEG && angle2 <= MAX_HOOD_ANGLE_DEG 
    ...
    private static boolean isTrajectoryValidForTopOpening(double hoodAngleDeg, double distance, double exitVelocity) {
        ...
        // If final vertical velocity is positive, it's still going UP when it hits the goal. Bounce-out!
        if (finalVerticalVelocity > 0) {
            return false; 
        }
        
        // 4. Calculate Entry Angle. We use absolute value because vy is negative (falling)
        double entryAngleDeg = Math.toDegrees(Math.abs(Math.atan2(finalVerticalVelocity, horizontalVelocity)));
